...
# Web3.0 탈중앙화 채팅 앱 매뉴얼

## 개요
이 매뉴얼에서는 IPFS, DHT, DID, PubSub와 같은 Web3.0 기술을 사용하여 Kotlin으로 Android 네이티브 앱을 개발하는 방법을 안내해 드리겠습니다. 이 앱을 통해 디자인 패턴(DTO, Repository, Provider)을 적용하면서 두 사용자가 중앙 서버 없이 안전하게 메시지를 주고받을 수 있습니다. 채팅 UI는 카카오톡, WhatsApp, iMessage와 같은 인기 메시징 앱의 인터페이스를 참고하여 대화가 순차적으로 진행될 수 있도록 구성합니다.
...
async function createLibp2p() {
  ...
  await libp2p.start();
  console.log(`Libp2p 시작, ID: ${peerId.toB58String()}`);
...
class Provider {
  ...
  listen() {
    this.libp2p.on('peer:connect', async (peerInfo) => {
      console.log(`피어와 연결됨 ${peerInfo.toB58String()}`);
    });
  }
  ...
  async receiveMessage(stream, sender) {
    ...
    console.log(`메시지 수신 - 발신자: ${sender}, 내용: ${message}`);
...
class Libp2pManager(private val context: Context, private val onMessageReceived: (String) -> Unit) {
    ...
    private fun initializeLibp2p() {
        GlobalScope.launch {
            withContext(Dispatchers.IO) {
                // Libp2p 구성 요소 초기화
                localPeerId = PeerId.fromBytes(byteArrayOf()) // PeerId 초기화 코드로 대체
    ...
    fun sendMessage(message: String) {
        GlobalScope.launch {
            withContext(Dispatchers.IO) {
                val remotePeerId = PeerId.fromBytes(byteArrayOf()) // 실제 PeerId로 대체
